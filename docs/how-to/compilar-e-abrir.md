...
cd macos
xcodegen generate
xcodebuild -scheme Breve -configuration Debug -derivedDataPath ./DerivedData.noindex -destination 'platform=macOS' build
killall Breve
open DerivedData.noindex/Build/Products/Debug/Breve.app
```

`xcodegen generate` copia o catálogo de `content/` para `macos/Resources/` e gera o `.xcodeproj`. Rode de novo se você criou arquivo Swift ou mudou `project.yml`. O `-derivedDataPath ./DerivedData.noindex` evita que o Spotlight indexe o `.app` local com o mesmo nome do app em Aplicativos.
...
```bash
killall Breve
open --env BREVE_DEFAULTS_SUITE=dev.fordevs.breve.setup-test-01 DerivedData.noindex/Build/Products/Debug/Breve.app
```
